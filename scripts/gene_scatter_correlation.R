...
disease_filt_list <- c("BLCA","ESCA","HNSC","LIHC","LUAD","LUSC")
...
for(disease_filt in disease_filt_list){
  ...
  txt <- paste("----------------------------",dim(scatter_matrix)[1],dim(scatter_matrix)[2],title_string,"P value",as.character(tt$p.value),"mean",as.character(exp(tt$estimate))[1],sep = "\n")
  write(txt,file = fp_string,append = TRUE)
