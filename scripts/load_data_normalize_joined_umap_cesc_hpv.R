...
#right now we are not saving the survival correctly because it is done in the forloop that goes through the hpv statuses
rm_list <- ls()
...
order_layer_2 <- quo(hpv_status == "Positive")
order_layer_3 <- quo(hpv_status == "Negative")
...
axis_numbers <- TRUE
show_p_vals <- TRUE 
order_fun <- "functions/hpv_order.r"
...
colors_file = "C:/Users/JAM526/pancan_fpkmuq/met_analysis/data/color_palettes/pos_neg.RDS"
...
cesc_hpv <- readr::read_tsv("C:/Users/JAM526/pancan_fpkmuq/met_analysis/data/cesc_hpv_exome.txt")

samps <- all_metadata$sample
for(j in 1:length(all_metadata$sample)){
  
  if(all_metadata$dataset[j] == "TCGA"){
    samps[j] <- substr(samps[j], 1, 12)
  }
  
}
all_metadata$sample <- samps

all_metadata <-  left_join(all_metadata,cesc_hpv,by = "sample")
surv_data <- readRDS(file.path(base_dir,data_dir,"gdc_survival.rds"))
surv_data <- surv_data[,c("sample","OS","OS.time")]

samps <- surv_data$sample
for(j in 1:length(surv_data$sample)){
  
  samps[j] <- substr(samps[j], 1, 12)
  
  
}
surv_data$sample <- samps
...
for(pathway in pathway_files){
  ...
  gene_table <- tibble(HGNC_Symbol = entrez_to_gene_name, Ensembl_ID = entrez_to_ensembl)
  ...
  rn <- rownames(wanted_tpm)
  for(j in 1:length(rn)){
    
    rn[j] <- substr(rn[j], 1, 12)
    
    
  }
  rownames(wanted_tpm) <- rn
  ...
  contrasting_colors <-c("#e41a1c", "#377eb8") %>% rev() #tolower(readRDS(file = colors_file )) #reading from the colors file (a list of color hex values)
  ...
  save_list <- c(save_list,list(first_scatter = umap_scatter))
  ...
  if(draw_lines){
    ...
    save_list <- c(save_list,list(first_scatter_lines = umap_scatter_lines))
    
  } else{}
  
  
  # PLOTTING THE UMAP OUTPUT COLORED BY CLUSTER
  title_string_hdbscan <- paste0("HDBSCAN of " ,pway_name_string)
  ...
  out_plot <- ggplot(joined_umap,aes(x=X1,y=X2)) + 
    ...
    labs(title = title_string_hdbscan, y="UMAP 2", x="UMAP 1",caption = caption_string,shape = plot_names[[quo_name(shape_selection)]])
  ...
  out_plot_cluster <- out_plot;
  save_list <- c(save_list,list(out_scatter_cluster = out_plot_cluster))
  ...
  if(run_stats){
    ...
    #MAKING THE bar PLOT
    title_string3 <- paste0(" Cluster Composition of ",title_string_hdbscan)
    ...
    cluster_comp_bp <- ggplot(freq_tab,aes(x = Cluster,y = n,label = fisher_exact_p))+fill_scale+geom_bar(stat = "identity")  +aes(fill =  !!sym(color_column_string))+
    ...
    save_list <- c(save_list,list(joined_umap = joined_umap))
    status_sign_symbol <- c("+","-")
    status_strings <- c("Positive","Negative")
    ########### Could do a little loop thing to reduce redundant code
    ####Survival of HPV positive
    saved_once <- FALSE
    for (j in 1:length(status_sign_symbol))  {
      title_string_surv <- paste0("Survival of HPV",status_sign_symbol[j]," ", 
                                  pway_name_string," ", 
                                  apply( title[1,] , 1 , paste , collapse = " " )) 
      
      joined_umap_hpv_filter <- dplyr::filter(joined_umap, !is.na(hpv_status) & hpv_status == status_strings[j] & Cluster != 0 )
      
      
      fit <- survfit(Surv(OS.time,OS) ~ Cluster,data = joined_umap_hpv_filter)
      save_list <- c(save_list,list(full_survival_fit = fit))
      
      if(any(joined_umap$Cluster == 0)){
        pal = ggcolor_hue_minus(max(strtoi(levels(joined_umap_hpv_filter$Cluster)))+1)
      } else{  pal = NULL}
      
      surv_plot_obj <- ggsurvplot(fit,data = joined_umap_hpv_filter, pval = TRUE,title = title_string_surv,ggtheme = theme_gray(),ylim = c(0,1),xlim = c(0,max(joined_umap_hpv_filter$OS.time,na.rm = TRUE)),palette = pal)
      ...
      pw <- pairwise_survdiff(Surv(OS.time,OS) ~ Cluster,data = joined_umap_hpv_filter,p.adjust.method = "none")
      ...
      surv_plot_arranged <-  arrangeGrob(grobs  = list(surv_plot_obj$plot,textGrob("Pairwise P-Values:"),pw_tab),heights = c(10,3,3)) 
      ...
      if(save_image){
        plots <- quos(surv_plot,umap_scatter,out_plot_cluster,umap_scatter_lines,cluster_comp_bp)
        for (w in plots){
          var_name <- w %>% quo_name()
          if(var_name %>%  exists()) {
            ...
            if (var_name == "surv_plot"){ 
              print(eval_tidy(w))
              fp_string <- file.path(base_dir,output_dir,paste(title_string,"_",var_name,status_strings[j],sep = ""))
  ...
  fp_string <- file.path(base_dir,output_dir,paste(title_string,"_gene_table",sep = ""))
  fp_string <- gsub(" ","_", fp_string,fixed = TRUE)
  fp_string <- paste(fp_string,".csv",sep = "")
  write_csv(x = gene_table, path = fp_string) 
