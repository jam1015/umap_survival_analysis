...
for(disease_filt in disease_filt_list){
  ...
  for(cluster_hm in cluster_hm_list){
    ...
    #meta_tpm_joined <- merge(metadata,tpm_df,by = "sample") #deleted
    ...
    meta_tpm_joined <- read_csv("C:\\Users\\JAM526\\pancan_fpkmuq\\met_analysis\\data\\hb_meta_tpm_joined.csv")
    
    tx1 <- tx1[tx1 %in% colnames(meta_tpm_joined)]
    tx2 <- tx2[tx2%in% colnames(meta_tpm_joined)]
    tx <- c(tx1,tx2)
    ...
    if(cluster_hm){title_string <- paste(title_string,"Clustered")}
    ...
    vec1 <- actual_scatter_matrix[,tx1] #first pathway
    vec2 <- actual_scatter_matrix[,tx2]  #second pathway
    actual_scatter_matrix$mean1 <- rowMeans(vec1)
    actual_scatter_matrix$mean2 <- rowMeans(vec2)
    ...
    out_plot <- ggplot(actual_scatter_matrix,aes(x=mean1,y = mean2))+
      ...
      geom_smooth(mapping = aes(x=mean1,y =mean2),method = "lm",color = "red")+stat_cor(method = "pearson")
    print(out_plot)
    
    #saving image of scatter plot
    ...
    cor <- rcorr(data.matrix(actual_scatter_matrix[,tx]))
  ...
  tt <- t.test(x = c(scatter_matrix[,tx1,drop = FALSE]))
  ...
  txt <- paste(title_string,"P value",as.character(tt$p.value),"mean",as.character(exp(tt$estimate))[1],sep = "\n")
  write(txt,file = fp_string,append = TRUE)
  
  
  #printing out the t test for nfe2l2 targets
  tt <- t.test(x = c(scatter_matrix[,tx2,drop = FALSE]))
  fp_string <- file.path(base_dir,output_dir)
  fp_string <- gsub(" ","_", fp_string,fixed = TRUE) 
